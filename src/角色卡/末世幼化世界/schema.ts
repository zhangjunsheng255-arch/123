...
const BaseStatSchema = z.object({
  当前: z.coerce.number().transform(v => _.clamp(v, 0, 9999)).prefault(100),
...
export const Schema = z.object({
  ...
  // ========== 主角核心系统 ==========
  主角: z.object({
    人物姓名: z.string().transform(v => v.trim()).prefault("写卡大师"),
    人物年龄: z.coerce.number().transform(v => _.clamp(v, 18, 60)).prefault(25),

    // 复用状态池模型
    HP: BaseStatSchema.extend({}).prefault({}),
    AP: BaseStatSchema.extend({
      当前: z.coerce.number().transform(v => _.clamp(v, 0, 9999)).prefault(50),
      上限: z.coerce.number().transform(v => _.clamp(v, 1, 9999)).prefault(50),
    }).prefault({}),

    当前等级: z.coerce.number().transform(v => _.clamp(v, 1, 9)).prefault(1),
    经验值: z.coerce.number().transform(v => _.clamp(v, 0, 999999)).prefault(0),
    晶核数量: z.coerce.number().transform(v => _.clamp(v, 0, 999999)).describe('世界通用货币/能源').prefault(0),

    当前装备: z.record(z.enum(["主手", "副手", "头部", "上装", "下装", "腿部", "足部", "饰品"]), z.string()).prefault({
      主手: "",
      副手: "",
      头部: "",
      上装: "",
    ...
    称号: z.record(z.string().describe('称号名'), z.boolean()).transform(data => _.pickBy(data, Boolean)).prefault({}),
  }).prefault({}),

  // ========== 主角状态系统 ==========
  状态: z.object({
    属性点: z.object({
      力量: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(5),
      感知: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(5),
      耐力: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(5),
      魅力: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(5),
      智力: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(5),
      敏捷: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(5),
      幸运: z.coerce.number().transform(v => _.clamp(v, 0, 100)).prefault(5),
    }).prefault({}),
  ...
  // ========== 背包系统 ==========
  背包: z.object({
    武器: z.record(z.string().describe('武器名'), BaseItemSchema.extend({
    ...
    装备: z.record(z.string().describe('装备名'), BaseItemSchema.extend({
    ...
    药品: z.record(z.string().describe('药品名'), BaseItemSchema.extend({
    ...
    道具: z.record(z.string().describe('道具名'), BaseItemSchema.extend({
  ...
  // ========== 基地建设系统 ==========
  基地: z.object({
    基地等级: z.coerce.number().transform(v => _.clamp(v, 0, 9)).prefault(0),

    控制统计: z.object({
      当前控制幼女总数: z.coerce.number().transform(v => _.clamp(v, 0, 999)).prefault(0),
      可控制上限: z.coerce.number().transform(v => _.clamp(v, 0, 999)).prefault(3),
    }).prefault({}),
    ...
    幼女分类与数量: z.object({
      战斗型幼女: z.coerce.number().transform(v => _.clamp(v, 0, 999)).prefault(0),
      探索搜集型幼女: z.coerce.number().transform(v => _.clamp(v, 0, 999)).prefault(0),
      生产种植型幼女: z.coerce.number().transform(v => _.clamp(v, 0, 999)).prefault(0),
      繁育辅助型幼女: z.coerce.number().transform(v => _.clamp(v, 0, 999)).prefault(0),
    }).prefault({}),

    任务系统: z.record(z.string().describe('任务名'), z.object({
  ...
  在场人物: z.record(z.string().describe('人物姓名'), z.object({
